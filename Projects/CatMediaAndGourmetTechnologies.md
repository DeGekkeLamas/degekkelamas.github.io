...
<font color="#d0d0d0">To program the generation of the grid, I had to define the rules by which the grid should be generated, such as pre-existing connections not being allowed, and then translate those rules to code.</font>\
<font color="#d0d0d0">The grid is not allowed to have any pre-made connections in it, so if when generating there are 2 nodes of the same type next to each other, then the node next to it must be a different type, to prevent an already existing match.</font>
